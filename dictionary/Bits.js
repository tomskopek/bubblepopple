...
BitString.prototype = {
  ...
  get: function (p, n) {
    ...
    if ((p % W) + n <= W) {
      return (
        (ORD[this.bytes[(p / W) | 0]] & BitString.MaskTop[p % W]) >>
        (W - (p % W) - n)
      );
...
/**
  ...
  */
export function FrozenTrie(data, directoryData, nodeCount) {
...
FrozenTrie.prototype = {
  init: function (data, directoryData, nodeCount) {
    ...
    this.directory = new RankDirectory(
      directoryData,
      data,
      nodeCount * 2 + 1,
      L1,
  ...
  getNodeByIndex: function (index) {
    ...
    var letter = String.fromCharCode(
      this.data.get(this.letterStart + index * 6 + 1, 5) + "a".charCodeAt(0)
    );
